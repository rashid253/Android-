...
import { onAuthState, signIn, signUp, resetPassword, signOutUser } from './auth.js';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Monitor auth state and toggle UI
  onAuthState(user => {
    if (user) {
  ...
  // Sign Up (admin/principal only)
  signupBtn.onclick = async () => {
    const email       = emailInput.value.trim();
    const pwd         = passwordInput.value;
    const displayName = prompt('Enter your name');
    try {
      await signUp(email, pwd, displayName);
  ...
  function bindSelection() {
    const boxes = Array.from(document.querySelectorAll('.sel'));     boxes.forEach(cb => {
      cb.onchange = () => {
  ...
  shareAtt.onclick=ev=>{
    ...
    window.open(`https://wa.me/?text=${encodeURIComponent([hdr,'',...lines,'',`Overall: ${pct}% | ${remark}`].join('\n'))}`,'_blank');
  };
