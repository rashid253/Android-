...
window.addEventListener('DOMContentLoaded', () => {
  // لوکل اسٹوریج سے اسٹوڈنٹس لوڈ کریں
  ...
  // اسٹوڈنٹس سیو کرنے کا فنکشن
  function saveStudents() {
    localStorage.setItem('students', JSON.stringify(students));
  }

  // چیک باکس سلکشن بائنڈ کرنا
  ...
  // نیا اسٹوڈنٹ ایڈ کرنے کا ہینڈلر
  addStudentBtn.onclick = ev => {
    ...
    if (!name || !adm || !parent || !contact || !occ || !addr) {
      return alert('تمام فیلڈز بھرنا ضروری ہیں۔');
    }
    if (!/^\d+$/.test(adm)) {
      return alert('Admission# صرف عدد ہونا چاہیے۔');
    }
    if (students.some(s => s.adm === adm)) {
      return alert(`Admission# ${adm} پہلے سے موجود ہے۔`);
    }
    if (!/^\d{7,15}$/.test(contact)) {
      return alert('Contact 7 سے 15 ہندسے پر مشتمل ہونا چاہیے۔');
    }
  ...
  // سیو ریجسٹریشن کا ہینڈلر
  saveRegBtn.onclick = ev => {
    ev.preventDefault();
    regSaved = true;
    ['editSelected','deleteSelected','selectAllStudents','saveRegistration'].forEach(id => {
